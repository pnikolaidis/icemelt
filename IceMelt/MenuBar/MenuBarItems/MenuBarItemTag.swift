...
struct MenuBarItemTag: Hashable, CustomStringConvertible {
    ...
    /// A Boolean value that indicates whether the item identified
    /// by this tag is a control item owned by IceMelt.
...
extension MenuBarItemTag {
    ...
    /// An array of tags for items representing IceMelt's control items.
    static let controlItems = ControlItem.Identifier.allCases.map { $0.tag }

    // MARK: Control Items

    /// The tag for IceMelt's control item for the "Visible" section.
    static let visibleControlItem = MenuBarItemTag(controlItem: .visible)

    /// The tag for IceMelt's control item for the "Hidden" section.
    static let hiddenControlItem = MenuBarItemTag(controlItem: .hidden)

    /// The tag for IceMelt's control item for the "Always-Hidden" section.
...
extension MenuBarItemTag.Namespace {
    /// The namespace for the "IceMelt" process.
    static let iceMelt = string(Constants.bundleIdentifier)
