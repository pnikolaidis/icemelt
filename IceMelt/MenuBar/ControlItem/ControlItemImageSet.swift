...
struct ControlItemImageSet: Codable, Hashable, Identifiable {
    enum Name: String, Codable, Hashable {
        ...
        // Upstream Ice's cube artwork, kept as an icon option under its own name.
        case iceCube = "Ice Cube"
        case iceMelt = "IceMelt"
...
extension ControlItemImageSet {
    /// The default image set for the IceMelt icon.
    static let defaultIceMeltIcon = ControlItemImageSet(
        name: .iceMelt,
        image: .catalog("IceMeltDroplet")
    )

    /// The image sets that the user can choose to display in the IceMelt icon.
    static let userSelectableIceMeltIcons = [
        ControlItemImageSet(
            name: .iceMelt,
            image: .catalog("IceMeltDroplet")
        ),
