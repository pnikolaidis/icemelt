...
extension Defaults {
    enum Key: String {
        // NOTE: The raw values below are the on-disk `UserDefaults` keys. The ones
        // still spelled "Ice" are frozen at the spelling IceMelt shipped with —
        // renaming them would silently reset existing users' settings.

        // MARK: General Settings
        case showIceMeltIcon = "ShowIceIcon"
        case iceMeltIcon = "IceIcon"
        case customIceMeltIconIsTemplate = "CustomIceIconIsTemplate"
        case useIceMeltBar = "UseIceBar"
        case iceMeltBarLocation = "IceBarLocation"
