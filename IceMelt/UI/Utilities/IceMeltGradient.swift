//
//  IceMeltGradient.swift
//  IceMelt
//

import SwiftUI

// MARK: - IceMeltGradient

/// A custom gradient.
struct IceMeltGradient: Codable, Hashable {
    ...
    func withAlpha(_ alpha: CGFloat) -> IceMeltGradient {
        let newStops = stops.map { $0.withAlpha(alpha) }
        return IceMeltGradient(stops: newStops)
    }
...
// MARK: IceMeltGradient Static Members
extension IceMeltGradient {
    /// The default menu bar tint gradient.
    static let defaultMenuBarTint = IceMeltGradient(stops: [
        ColorStop.white(location: 0),
        ColorStop.black(location: 1),
    ])
}

// MARK: - IceMeltGradient.ColorStop
...
// MARK: IceMeltGradient.ColorStop: Codable
extension IceMeltGradient.ColorStop: Codable {
    ...
    init(from decoder: any Decoder) throws {
        ...
        self.color = try container.decode(IceMeltColor.self, forKey: .color).cgColor
        self.location = try container.decode(CGFloat.self, forKey: .location)
    }

    func encode(to encoder: any Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(IceMeltColor(cgColor: color), forKey: .color)
