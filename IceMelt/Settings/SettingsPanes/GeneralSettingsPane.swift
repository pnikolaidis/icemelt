...
struct GeneralSettingsPane: View {
    ...
    @State private var isImportingCustomIceMeltIcon = false
    ...
    // MARK: IceMelt Icon Options

    @ViewBuilder
    private var iceMeltIconOptions: some View {
        showIceMeltIcon
        if settings.showIceMeltIcon {
            iceMeltIconPicker
        }
    }

    @ViewBuilder
    private var showIceMeltIcon: some View {
        Toggle("Show IceMelt icon", isOn: $settings.showIceMeltIcon)
            .annotation("Click to show hidden menu bar items. Right-click to access IceMelt's settings.")
    }

    @ViewBuilder
    private var iceMeltIconPicker: some View {
        let labelKey = LocalizedStringKey("IceMelt icon")

        IceMeltMenu(labelKey) {
            Picker(labelKey, selection: $settings.iceMeltIcon) {
                ForEach(ControlItemImageSet.userSelectableIceMeltIcons) { imageSet in
                    Button {
                        settings.iceMeltIcon = imageSet
                    } label: {
                        iceMeltIconMenuItem(for: imageSet)
                    }
                    .tag(imageSet)
                }
                if let lastCustomIceMeltIcon = settings.lastCustomIceMeltIcon {
                    Button {
                        settings.iceMeltIcon = lastCustomIceMeltIcon
                    } label: {
                        iceMeltIconMenuItem(for: lastCustomIceMeltIcon)
                    }
                    .tag(lastCustomIceMeltIcon)
                }
            }
            .pickerStyle(.inline)
            .labelsHidden()
            ...
            Button("Choose image…") {
                isImportingCustomIceMeltIcon = true
            }
        } title: {
            iceMeltIconMenuItem(for: settings.iceMeltIcon)
        }
        .annotation("Choose a custom icon to show in the menu bar.")
        .fileImporter(
            isPresented: $isImportingCustomIceMeltIcon,
            ...
            do {
                ...
                if url.startAccessingSecurityScopedResource() {
                    ...
                    settings.iceMeltIcon = ControlItemImageSet(name: .custom, image: .data(data))
                }
            } catch {
        ...
        if case .custom = settings.iceMeltIcon.name {
            Toggle("Custom icon uses dynamic appearance", isOn: $settings.customIceMeltIconIsTemplate)
                .annotation {
                    Text(
                        """
    ...
    @ViewBuilder
    private func iceMeltIconMenuItem(for imageSet: ControlItemImageSet) -> some View {
        Label {
    ...
    // MARK: IceMelt Bar Options

    @ViewBuilder
    private var iceMeltBarOptions: some View {
        useIceMeltBar
        if settings.useIceMeltBar {
            iceMeltBarLocationPicker
        }
    }

    @ViewBuilder
    private var useIceMeltBar: some View {
        Toggle("Use IceMelt Bar", isOn: $settings.useIceMeltBar)
            .annotation("Show hidden menu bar items in a separate bar below the menu bar.")
    }

    @ViewBuilder
    private var iceMeltBarLocationPicker: some View {
        IceMeltPicker("Location", selection: $settings.iceMeltBarLocation) {
            ForEach(IceMeltBarLocation.allCases) { location in
                Text(location.localized).tag(location)
            }
        }
        .annotation {
            switch settings.iceMeltBarLocation {
            case .dynamic:
                Text("The IceMelt Bar's location changes based on context.")
            case .mousePointer:
                Text("The IceMelt Bar is centered below the mouse pointer.")
            case .iceMeltIcon:
                Text("The IceMelt Bar is centered below the IceMelt icon.")
            }
        }
    }
    ...
    private var rehideStrategyPicker: some View {
        VStack {
            IceMeltPicker("Strategy", selection: $settings.rehideStrategy) {
