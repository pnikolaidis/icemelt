...
extension HIDEventManager {
    ...
    private func handleSmartRehide(with event: NSEvent, appState: AppState, screen: NSScreen) {
        ...
        // Make sure clicking the IceMelt icon doesn't trigger rehide.
        if let iceMeltIcon = appState.menuBarManager.controlItem(withName: .visible) {
            guard event.window !== iceMeltIcon.window else {
                return
            }
        }

        // Only continue if the click is not inside the IceMelt Bar, at
        // least one section is visible, and the mouse is not inside
        // the menu bar.
        guard
            event.window !== appState.menuBarManager.iceMeltBarPanel,
    ...
    private func handleShowOnHover(appState: AppState, screen: NSScreen) {
        ...
        if hiddenSection.isHidden {
            ...
            guard
                !isMouseInsideMenuBar(appState: appState, screen: screen),
                !isMouseInsideIceMeltBar(appState: appState)
            else {
                return
            }
            Task {
    ...
    private func handlePreventShowOnHover(with event: NSEvent, appState: AppState, screen: NSScreen) {
        ...
        if isMouseInsideMenuBarItem(appState: appState, screen: screen) {
            ...
            case .leftMouseDown:
                ...
                if isMouseInsideIceMeltIcon(appState: appState) {
                    break
                }
                return
            case .rightMouseDown:
...
extension HIDEventManager {
    ...
    func isMouseInsideMenuBar(appState: AppState, screen: NSScreen) -> Bool {
        // IceMelt icon must be vertically visible. Otherwise, we can infer
        // that the menu bar is hidden and the mouse is not inside.
        guard
            let iceMeltIcon = appState.menuBarManager.controlItem(withName: .visible),
            let iceMeltIconFrame = iceMeltIcon.frame,
            iceMeltIconFrame.maxY <= screen.frame.maxY,
            let mouseLocation = MouseHelpers.locationAppKit
        else {
            return false
        }
    ...
    /// A Boolean value that indicates whether the mouse pointer is within
    /// the bounds of the IceMelt Bar panel.
    func isMouseInsideIceMeltBar(appState: AppState) -> Bool {
        guard let mouseLocation = MouseHelpers.locationAppKit else {
            return false
        }
        let panel = appState.menuBarManager.iceMeltBarPanel
        // Pad the frame to be more forgiving if the user accidentally
        // moves their mouse outside of the IceMelt Bar.
        let paddedFrame = panel.frame.insetBy(dx: -15, dy: -15)
        return paddedFrame.contains(mouseLocation)
    }

    /// A Boolean value that indicates whether the mouse pointer is within
    /// the bounds of the IceMelt icon.
    func isMouseInsideIceMeltIcon(appState: AppState) -> Bool {
        guard
            let visibleSection = appState.menuBarManager.section(withName: .visible),
            let iceMeltIconFrame = visibleSection.controlItem.frame,
            let mouseLocation = MouseHelpers.locationAppKit
        else {
            return false
        }
        return iceMeltIconFrame.contains(mouseLocation)
    }
}
